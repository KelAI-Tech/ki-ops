...
# ---------------------------------------------------------------------------
# unexecuted_from_ledger (explained-recon input)
# ---------------------------------------------------------------------------


def _submit_at(submit_id, env, payloads, results, *, ts):
    return Submit(
        submit_id=submit_id,
        submitted_at=ts,
        env=env,
...
TS2 = datetime(2026, 9, 8, 18, 0, tzinfo=timezone.utc)  # a later forced re-run


def test_unexecuted_partial_fill_and_full_fill():
    submits = [
        _submit_at(
            "s1",
            "UAT",
            [
                {"symbol": "AAPL.US", "side": "BUY", "quantity": 100.0},
                {"symbol": "MSFT.US", "side": "BUY", "quantity": 30.0},
            ],
            [{"orderId": "a1", "success": True}, {"orderId": "m1", "success": True}],
            ts=TS,
        ),
    ]
    working = [
        _wo("s1", "a1", "AAPL.US", "BUY", 100, filled=60),
        _wo("s1", "m1", "MSFT.US", "BUY", 30, filled=30),  # fully filled → drops out
    ]
    assert unexecuted_from_ledger(submits, working, env="UAT") == {"AAPL": D("40")}


def test_unexecuted_counts_rejected_orders_in_full():
    submits = [
        _submit_at(
            "s1",
            "UAT",
            [{"symbol": "NKE.US", "side": "SELL", "quantity": 50.0}],
            [{"orderId": "n1", "success": False}],
            ts=TS,
        ),
    ]
    working = [_wo("s1", "n1", "NKE.US", "SELL", 50, filled=0)]
    assert unexecuted_from_ledger(submits, working, env="UAT") == {"NKE": D("-50")}


def test_unexecuted_dedups_resent_rejections():
    """Rejected then re-sent (and rejected again): only the latest attempt
    counts — the true shortfall is one lot, not two (today's live UAT sells)."""
    submits = [
        _submit_at(
            "morning",
            "UAT",
        ...
        _submit_at(
            ...
            "UAT",
            [{"symbol": "NKE.US", "side": "SELL", "quantity": 50.0}],
            [{"orderId": "n2", "success": False}],
            ts=TS2,
        ),
    ]
    working = [
        _wo("morning", "n1", "NKE.US", "SELL", 50, filled=0),
        _wo("retry", "n2", "NKE.US", "SELL", 50, filled=0),
    ]
    assert unexecuted_from_ledger(submits, working, env="UAT") == {"NKE": D("-50")}


def test_unexecuted_rejection_superseded_by_later_accepted_order():
    """Morning rejection re-sent and accepted with a partial fill: the accepted
    order's own remainder is the whole story; the stale rejection must not add."""
    submits = [
        _submit_at(
            "morning",
            "UAT",
        ...
        _submit_at(
            ...
            "UAT",
            [{"symbol": "NKE.US", "side": "SELL", "quantity": 50.0}],
            [{"orderId": "n2", "success": True}],
            ts=TS2,
        ),
    ]
    working = [
        _wo("morning", "n1", "NKE.US", "SELL", 50, filled=0),
        _wo("retry", "n2", "NKE.US", "SELL", 50, filled=20),
    ]
    assert unexecuted_from_ledger(submits, working, env="UAT") == {"NKE": D("-30")}


def test_unexecuted_env_filter_and_source_symbol_vocabulary():
    submits = [
        _submit_at(
            "s1",
            "UAT",
            # Resolution renamed the outgoing symbol; the recon vocabulary is
            # the ORIGINAL (ds2) spelling carried on sourceSymbol.
            [{"symbol": "BF/B.US", "sourceSymbol": "BFB.US", "side": "BUY", "quantity": 10.0}],
            [{"orderId": "b1", "success": True}],
            ts=TS,
        ),
        _submit_at(
            "p1",
            "PROD",
            [{"symbol": "AAPL.US", "side": "BUY", "quantity": 99.0}],
            [{"orderId": "pa1", "success": True}],
            ts=TS,
        ),
    ]
    working = [
        _wo("s1", "b1", "BF/B.US", "BUY", 10, filled=0),
        _wo("p1", "pa1", "AAPL.US", "BUY", 99, filled=0),
    ]
    assert unexecuted_from_ledger(submits, working, env="UAT") == {"BFB": D("10")}
